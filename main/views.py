...
class PatientUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context_data["title"] = (
            patient_item.last_name
            + " "
            + patient_item.first_name
            + " "
            + patient_item.surname
        )
        # Создаем формсет для записей на прием к врачам
        PatientFormset = inlineformset_factory(Patient, Appoint, AppointForm, extra=3)
        if self.request.method == "POST":
            # В случае, если обновляем данные
            context_data["formset"] = PatientFormset(
    ...
    def form_valid(self, form):
        ...
        if form.is_valid() and formset.is_valid():
            ...
            return render(
                self.request, self.get_context_data(form=form, formset=formset)
            )
    ...
    def get_form_class(self):
        ...
        if user == self.object.owner:
            return PatientForm
        if (
            user.has_perm("main.can_edit_email")
            and user.has_perm("main.can_edit_photo")
            and user.has_perm("main.can_edit_birthday")
        ):
...
class AppointDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["title"] = (
            f"Запись пациента {appoint_item.patient.last_name} {appoint_item.patient.first_name}"
        )
        return context
...
class AppointUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        appoint_item = self.get_object()
        context["title"] = (
            f"Редактирование записи пациента {appoint_item.patient.last_name} {appoint_item.patient.first_name}"
        )
        return context
...
class ResultUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        """Определяем переход при удачном завершении"""
        return reverse("main:result_detail", args=[self.kwargs.get("pk")])
